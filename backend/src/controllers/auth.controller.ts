...
export class AuthController {
  ...
  static uploadAvatar = asyncHandler(async (req: Request, res: Response<AuthResponse>) => {
    ...
    try {
      ...
      return res.json({
        ...
        data: { user: userResponse, token: req.headers.authorization?.split(' ')[1] || '' }
      });
    } catch (error: any) {
  ...
  static getUserEmail = asyncHandler(async (req: Request, res: Response<ApiResponse>) => {
    ...
    try {
      ...
      return res.status(500).json({
        success: false,
        message: error.message || 'Có lỗi xảy ra khi lấy thông tin email'
      });
    }
  });
