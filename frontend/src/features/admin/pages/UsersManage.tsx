...
function Modal({ title, onClose, children }: { title: string; onClose: () => void; children: React.ReactNode }) {
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="w-full max-w-xl overflow-hidden rounded-2xl border border-white/10 bg-[#1b2a3a] shadow-2xl">
      ...
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <input
            ...
            className="w-64 rounded-full border border-white/15 bg-white/10 px-4 py-2 text-sm text-white outline-none placeholder:text-white/60"
          />
          <select
            value={roleFilter}
            onChange={(e) => setRoleFilter(e.target.value as any)}
            className="rounded-lg bg-white/90 px-3 py-2 text-sm text-slate-900"
          >
          ...
          <button onClick={() => setOpenCreate(true)} className="rounded-lg px-3 py-2 text-sm font-medium ring-1 ring-white/20 hover:bg-white/10 text-white">Tạo tài khoản</button>
        </div>
      </div>
