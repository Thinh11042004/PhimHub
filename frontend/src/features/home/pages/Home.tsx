...
function WideCard({ it, cta = "▶ Xem" }: { it: Item; cta?: string }) {
  ...
  const handleFavoriteClick = async (e: React.MouseEvent) => {
    ...
    const slug = it.id;
    favoritesDebug.log('WideCard favorite click started', { itemId: slug, isFavorited: isFavorited });
    ...
    if (token) {
      ...
      try {
        ...
        favoritesDebug.log(`WideCard attempting to ${action} favorite`, { itemId: slug });
        ...
        favoritesDebug.log(`WideCard successfully ${action}ed favorite`, { itemId: slug });
        setIsFavorited(!isFavorited);
        window.dispatchEvent(new CustomEvent('favoritesUpdated'));
      } catch (error) {
        console.error('Error updating favorites:', error);
        favoritesDebug.log('WideCard error updating favorites', error);
      ...
      if (isInFavorites) {
        ...
        favoritesDebug.log('WideCard removed from localStorage favorites', { itemId: slug });
      } else {
        // Add to favorites
        const movieData = {
          id: slug,
        ...
        favoritesDebug.log('WideCard added to localStorage favorites', { itemId: slug });
      }
...
export default function Home() {
  ...
  const getCurrentSlug = () => {
    ...
    const uploadedSlugs = ["ke-huy-diet", "ngoi-truong-xac-song", "thon-phe-tinh-khong", "than-den-oi-uoc-di"];
    
    if (current.title.toLowerCase().includes("kẻ hủy diệt") || current.title.toLowerCase().includes("terminator")) {
      return "ke-huy-diet";
    } else if (current.title.toLowerCase().includes("ngôi trường xác sống") || current.title.toLowerCase().includes("zombie")) {
      return "ngoi-truong-xac-song";
    } else if (current.title.toLowerCase().includes("thôn phệ tinh không") || current.title.toLowerCase().includes("perfect world")) {
  ...
  const handleAddToFavorites = async () => {
    ...
    favoritesDebug.log('Hero favorite click started', { itemId: slug, isFavorited: favorites.has(slug) });
    ...
    if (token) {
      ...
      try {
        const { InteractionsApi } = await import('../../../services/movies/interactions');
        const action = favorites.has(slug) ? 'remove' : 'add';
        
        favoritesDebug.log(`Hero attempting to ${action} favorite`, { itemId: slug });
        ...
        favoritesDebug.log(`Hero successfully ${action}ed favorite`, { itemId: slug });
        window.dispatchEvent(new CustomEvent('favoritesUpdated'));
      } catch (error) {
        console.error('Error updating favorites:', error);
        favoritesDebug.log('Hero error updating favorites', error);
        alert('Không thể cập nhật yêu thích. Vui lòng thử lại.');
      }
    } else {
      // Fallback to localStorage for non-authenticated users
      ...
      if (favorites.has(slug)) {
        ...
        favoritesDebug.log('Hero removed from localStorage favorites', { itemId: slug });
      } else {
        // Thêm vào yêu thích
        const movieData = {
          id: slug,
        ...
        localStorage.setItem('phimhub:favorites', JSON.stringify(favoritesData));
        setFavorites(prev => new Set([...prev, slug]));
        favoritesDebug.log('Hero added to localStorage favorites', { itemId: slug });
      }
    }
  };
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex rounded-full bg-gray-800/60 backdrop-blur-md border border-white/20 overflow-hidden">
                  <button
                    ...
                    title={isCurrentInFavorites() ? "Bỏ yêu thích" : "Yêu thích"}
                    aria-label={isCurrentInFavorites() ? "Bỏ yêu thích" : "Yêu thích"}
                  >
                    <span className="text-xl">{isCurrentInFavorites() ? '❤️' : '🤍'}</span>
                  </button>
                  ...
                  <button
                    ...
                    title={isCurrentInWatchlist() ? "Xóa khỏi danh sách" : "Thêm vào danh sách"}
                    aria-label={isCurrentInWatchlist() ? "Xóa khỏi danh sách" : "Thêm vào danh sách"}
                  >
