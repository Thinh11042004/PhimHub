...
import { 
  ...
  getEpisodesList, 
  debugEpisodesData 
} from "../../../utils/episodesDataHandler";
...
export default function WatchSeries() {
  ...
  const SIMPLE_MP4 = "https://www.w3schools.com/html/mov_bbb.mp4";
  ...
  // Get episodes data with fallback logic
  const episodesData = getEpisodesData(detail);
  const activeServer = getActiveServer(episodesData, serverKey);
  const activeEpisode = getActiveEpisode(activeServer, sp);
  const providerStream = activeEpisode?.link_m3u8 || activeEpisode?.file;

  // Debug logging for episodes data
  debugEpisodesData(detail, episodesData, activeServer, activeEpisode);
      ...
      <section className="mx-auto w-full max-w-6xl px-3 md:px-4 py-6">
        <div className="rounded-2xl bg-white/5 p-4 ring-1 ring-white/10 backdrop-blur-sm md:p-6">
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
            {getEpisodesList(activeServer).map((ep, index) => {
        ...
        {/* Bình luận */}
        <div className="mt-4 rounded-2xl bg-white/5 p-4 ring-1 ring-white/10 backdrop-blur-sm" id="comments-section">
