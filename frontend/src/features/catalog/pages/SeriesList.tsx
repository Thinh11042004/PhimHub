...
function SeriesPosterCard({ s }: { s: Series }) {
  return (
    <PosterCard
      series={s}
      size="medium"
...
export default function SeriesList() {
  ...
  const slice = useMemo(() => {
    ...
    if (genres.length) arr = arr.filter(s => genres.some(g => s.genres.includes(g)));
    ...
    <div className="w-full space-y-6 px-4 md:px-6 lg:px-8 xl:px-12">
          ...
          {itemsAll.length > 0 && (
            <div className="flex items-center gap-4 text-xs text-white/50">
              <div className="flex items-center gap-1">
                ...
                <span>Mới nhất: {Math.max(...itemsAll.map(s => s.year))}</span>
              </div>
              <div className="flex items-center gap-1">
                <div className="h-2 w-2 rounded-full bg-blue-500"></div>
                <span>Thể loại: {new Set(itemsAll.flatMap(s => s.genres)).size}</span>
              </div>
            </div>
          )}
        </div>
          ...
          <div className="space-y-6">
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex flex-wrap gap-2">
                {[
                  {k:"all",label:"Tất cả"},
                  {k:"ongoing",label:"Đang chiếu"},
                  {k:"completed",label:"Hoàn thành"},
                ].map(s=>(
                  <Chip key={s.k} active={status===s.k} onClick={()=>setStatus(s.k as any)}>{s.label}</Chip>
                ))}
              </div>
            </div>
      ...
      {!loading && !fillingLoading && slice.length === 0 && (
        <div className="rounded-2xl bg-dark-800/50 p-12 text-center ring-1 ring-dark-600/50">
          ...
          <h3 className="mb-3 text-xl font-bold text-white">Chưa có phim bộ nào</h3>
          <p className="mb-6 text-white/70 max-w-md mx-auto">
            Hiện tại chưa có phim bộ nào được upload lên hệ thống. 
            Vui lòng liên hệ quản trị viên để thêm nội dung.
          </p>
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <button 
              onClick={() => window.location.href = '/admin/upload-series'}
      ...
      {!loading && !fillingLoading && slice.length === 0 && (
        <div className="rounded-2xl bg-dark-800/50 p-12 text-center ring-1 ring-dark-600/50">
          ...
          <h3 className="mb-2 text-lg font-semibold text-white">Không tìm thấy phim bộ nào</h3>
