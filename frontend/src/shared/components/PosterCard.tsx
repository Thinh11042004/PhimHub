...
export default function PosterCard({
  ...
  const handleFavoriteClick = async (e: React.MouseEvent) => {
    ...
    if (token) {
      ...
      const { useLoginRequired } = await import('../hooks/useLoginRequired');
      // We need to get the hook instance from the component that uses PosterCard
      // For now, we'll use a different approach - dispatch a custom event
      window.dispatchEvent(new CustomEvent('showLoginRequired', { 
        detail: { 
  ...
  const handleAddToListClick = async (e: React.MouseEvent) => {
    ...
    if (!token) {
      ...
      window.dispatchEvent(new CustomEvent('showLoginRequired', { 
        detail: { 
          title: "Yêu cầu đăng nhập", 
          message: "Bạn cần đăng nhập để thêm vào danh sách" 
        } 
      }));
      return;
    }
  ...
  return (
    ...
    >
      ...
      <div className="mt-3">
        <h3 className="text-white font-semibold text-sm line-clamp-1 group-hover:text-primary-300 transition-colors duration-200">
          {item.title}
        </h3>
      </div>
