...
export default function AccountLayout() {
  ...
  return (
    <div className="mx-auto grid max-w-6xl grid-cols-1 gap-4 px-3 py-6 md:grid-cols-[1fr,280px] md:px-4">
      {/* Content */}
      ...
      {/* Sidebar - Right side */}
      <aside className="space-y-4 order-1 md:order-2 bg-white/5 backdrop-blur-sm rounded-2xl p-4 ring-1 ring-white/10">
        {/* User Info Card */}
        ...
        <button
          onClick={logout}
          className="w-full rounded-xl bg-red-500/20 hover:bg-red-500/30 text-red-300 hover:text-red-200 px-4 py-3 transition-all duration-200 flex items-center justify-center gap-2"
        >
          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
function NavItem({ to, children, icon }: { to: string; children: React.ReactNode; icon?: React.ReactNode }) {
  return (
    <NavLink
      to={to}
      className={({ isActive }) =>
        `block rounded-xl px-4 py-3 ring-1 ring-white/10 transition-all duration-200 flex items-center gap-3 ${
          isActive 
