...
function mapToMovieDetail(resp: BackendMovieResponse): MovieDetail {
    ...
    episodesByServer.forEach((serverEpisodes, serverName) => {
      episodes.push({
        ...
        server_data: serverEpisodes.map(ep => {
          ...
          return {
            name: ep.title || ep.ep || `Tập ${episodeNum}`,
            slug: `tap-${episodeNum}`,
            filename: ep.title || ep.ep || `Tập ${episodeNum}`,
            link_m3u8: ep.episode_url || ep.link_m3u8 || "https://demo.unified-streaming.com/k8s/features/stable/video/tears-of-steel/tears-of-steel.ism/.m3u8",
  ...
  const stream = {
    hls: data.episodes && data.episodes.length > 0 
      ? data.episodes[0].episode_url || data.episodes[0].link_m3u8 || "https://demo.unified-streaming.com/k8s/features/stable/video/tears-of-steel/tears-of-steel.ism/.m3u8"
      : "https://demo.unified-streaming.com/k8s/features/stable/video/tears-of-steel/tears-of-steel.ism/.m3u8",
    mp4: "https://demo.unified-streaming.com/k8s/features/stable/video/tears-of-steel/tears-of-steel.ism/.m3u8"
  };
  ...
  return {
    ...
    duration: isSeries ? 0 : (data.slug === 'toi-la-sat-thu-nhung-chi-so-manh-hon-han-dung-si' ? 25 : (data.duration || 0)), // Override duration for specific movie
...
export class LocalBackendProvider implements MovieProvider {
  async getMovieById(slug: string): Promise<MovieDetail> {
    try {
      ...
      // Nếu là phim bộ, gọi API episodes riêng
      if (json.data.is_series) {
        try {
          console.log('🎬 Fetching episodes for series:', json.data.id);
          const episodesRes = await fetch(`${BASE_URL}/episodes/movie/${json.data.id}`);
          console.log('🎬 Episodes API response status:', episodesRes.status);
          
          if (episodesRes.ok) {
            const episodesJson = await episodesRes.json();
            console.log('🎬 Episodes API response:', episodesJson);
            
            if (episodesJson.success && episodesJson.data.episodes) {
              console.log('🎬 Fetched episodes from API:', episodesJson.data.episodes.length);
              console.log('🎬 First episode URL:', episodesJson.data.episodes[0]?.episode_url);
              json.data.episodes = episodesJson.data.episodes;
            } else {
              console.warn('⚠️ Episodes API returned no episodes');
            }
          } else {
            console.error('❌ Episodes API failed:', episodesRes.status, episodesRes.statusText);
          }
        } catch (episodesError) {
  ...
  async switchVersion(id: string, versionKey: string) {
    try {
      ...
      return { 
        hls: selectedEpisode?.link_m3u8 || selectedEpisode?.episode_url || "https://demo.unified-streaming.com/k8s/features/stable/video/tears-of-steel/tears-of-steel.ism/.m3u8",
        subtitles: [
          { label: "Tiếng Việt", lang: "vi", src: "", default: true },
          { label: "English", lang: "en", src: "" }
        ]
